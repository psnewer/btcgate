...
class Future_Handler(object):
	...
	self.contract = contract
        ...
        self.goods_rt_std = contract_params['goods_rt']
    ...
    def get_flag(self):
        ...
        if self.forward_follow_alarm and not self.backward_follow_alarm:
            _T = abs(float(self.backward_position_size) / (float(self.forward_position_size)*(1.0+self.balance_overflow/self.forward_goods)))
        elif self.backward_follow_alarm and not self.forward_follow_alarm:
            _T = abs(float(self.forward_position_size) / (float(self.backward_position_size)*(1.0+self.balance_overflow/self.backward_goods)))

        _S = -1
        if _T >= 0:
            if self.forward_follow_alarm and not self.backward_follow_alarm:
                _S = -(self.balance_overflow + self.backward_goods)/self.forward_goods
            elif self.backward_follow_alarm and not self.forward_follow_alarm:
                _S = -(self.balance_overflow + self.forward_goods)/self.backward_goods

        if _T >= 0:
            self.goods_rt = 1.0 - _T * 0.2
        else:
        ...
        if self.forward_follow_alarm and self.backward_follow_alarm:
            ...
            if -self.balance_overflow/self.forward_goods > self.goods_rt*self.onset_rt and self.forward_stable_price:
                self.forward_gap_balance = True
                self.backward_gap_balance = False
            ...
            if -self.balance_overflow/self.backward_goods > self.goods_rt*self.onset_rt and self.backward_stable_price:
                self.forward_gap_balance = False
                self.backward_gap_balance = True
        ...
        self.catch_neg_rt = 0.3 + 0.1*_T
        self.catch_pos_rt = 0.3 + 0.1*_T
        self.forward_catch = False
        self.backward_catch = False
        print (_S)
        print (_T)
        if self.catch and _T >= 0:
            if self.forward_follow_alarm:
                if _S > 1.0 - self.catch_neg_rt * _T:
...
#                if forward_neg > 0 and self.backward_gap > 0.0 and (-self.backward_position_size/forward_neg > self.catch_rt or (abs(self.backward_position_size)>=self.limit_size and -self.backward_position_size/forward_neg > self.catch_thresh_rt)):
                ...
                elif _S < 1.0 - self.catch_pos_rt * _T:
                    if not self.forward_gap_balance and not self.backward_gap_balance and self.forward_stable_price:
