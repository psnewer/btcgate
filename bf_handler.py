...
class Future_Handler(object):
    ...
    def get_flag(self):
        ...
        delta = self.forward_position_size - self.backward_position_size
        ...
        print (self.backward_position_size)
        print (self.backward_trigger_liq)
        print (self.backward_leverage)
        print (self.backward_liq_price)
        if self.backward_position_size < 0 and (self.backward_trigger_liq < 0 or (self.backward_trigger_liq <= (1.0-1.0/self.backward_leverage*0.5)*backward_liq_price or self.backward_trigger_liq >= (1.0-1.0/self.backword_leverage*0.2)*backward_liq_price)):
            self.backward_liq_flag = True
        else:
            self.backward_liq_flag = False
        print (self.backward_liq_flag)
    ...
    def put_position(self):
        ...
        for order in self.forward_orders:
            ...
            if order_size > 0:
                ...
                if self.ask_1 < self.forward_entry_price:
                    ...
                    forward_api_instance.cancel_futures_order(order_id)
                    forward_api_instance.create_futures_order(FuturesOrder(contract=self.contract,size = -self.forward_position_size, price = self.ask_1,tif='poc'))
        ...
        if not forward_reduce_clear:
            if self.ask_1 >= self.forward_entry_price and self.forward_position_size > 0:
                ...
                forward_api_instance.create_futures_order(FuturesOrder(contract=self.contract,size = -self.forward_position_size, price = self.forward_entry_price*(1 + self.profit_gap), tif='poc'))
                ...
                if self.bid_1 > self.backward_entry_price:
                    if self.backward_gap_alarm:
                        ...
                        if balance_overflow > 0:
                            ...
                            _size = max(int(self.backward_position_size * (balance_overflow/abs(un_pnl)/self.N_contract)),self.backward_position_size)
                    ...
                    backward_api_instance.cancel_futures_order(order_id)
                    backward_api_instance.create_futures_order(FuturesOrder(contract=self.contract,size = -self.backward_position_size, price = self.bid_1,tif='poc'))
        if not backward_increase_clear:
            if not self.backward_position_alarm and not self.backward_delta_alarm and not self.backward_follow_alarm and not self.backward_gap_alarm:
                backward_api_instance.create_futures_order(FuturesOrder(contract=self.contract,size = -self.tap * self.backward_leverage, price = self.bid_1,tif='poc'))
        if not backward_reduce_clear:
            if self.bid_1 <= self.backward_entry_price and self.backward_position_size < 0:
                ...
                backward_api_instance.create_futures_order(FuturesOrder(contract=self.contract,size = -self.backward_position_size, price = self.backward_entry_price*(1 + self.profit_gap), tif='poc'))
        ...
        if self.backward_liq_flag:
            ...
            self.forward_trigger_jia_id = forward_api_instance.create_price_triggered_order(FuturesPriceTriggeredOrder(initial=FuturesInitialOrder(contract='BTC_USD',size=0,price=str(0),close=True,tif='ioc',text='api'),trigger=FuturesPriceTrigger(strategy_type=0,price_type=1,rule=1,price=str(round(backward_liq_price*(1.0-0.4*1.0/self.backward_leverage),2)),expiration=315360)))._id
